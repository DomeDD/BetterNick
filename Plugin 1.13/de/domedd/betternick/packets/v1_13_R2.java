...
public class v1_13_R2 implements Listener {
		...
		public v1_13_R2(BetterNick main) {
			this.pl = main;
		}
		...
		public static void respawn(Player player) {
			...
			PacketPlayOutRespawn packet = new PacketPlayOutRespawn(DimensionManager.a(((CraftPlayer) player).getWorld().getEnvironment().getId()), EnumDifficulty.getById(((CraftPlayer) player).getWorld().getDifficulty().getValue()), WorldType.getType(((CraftPlayer) player).getWorld().getWorldType().getName()), EnumGamemode.getById(((CraftPlayer) player).getGameMode().getValue()));
			sendPacket(player, packet);
		}
		public static void sendActionBar(Player player, String message) {
			if(player.isOnline()) {
			    PacketPlayOutTitle packet = new PacketPlayOutTitle(PacketPlayOutTitle.EnumTitleAction.ACTIONBAR, IChatBaseComponent.ChatSerializer.a("{\"text\": \"" + message + "\"}"));
			    sendPacket(player, packet);
			} else {
